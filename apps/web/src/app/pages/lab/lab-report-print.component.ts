...
import { LabReportRecordDetail } from '@hospital/shared';
...
export class LabReportPrintComponent implements OnInit, OnDestroy {
  ...
  reportBody(): string {
    ...
    if (typeof r.fieldValues['reportBody'] === 'string') {
      return r.fieldValues['reportBody'];
    }

    const values = r.fieldsSchema
      .map((f) => (r.fieldValues[f.key] ?? '').trim())
      .filter((v) => v.length > 0);
